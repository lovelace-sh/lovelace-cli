...
def cli(ctx, upgrade, version, verbose, log_file):
    """Lovelace CLI - Sync files between Cloudflare R2 and local"""
    ...
    # Check for updates before running commands (but not for --help or no command)
    if ctx.invoked_subcommand is not None:
        upgrader = Upgrader()
        needs_update, current, latest = upgrader.check_for_update()
        if needs_update and latest:
            click.echo(f"⚠️  Update available: v{current} → v{latest}", err=True)
            click.echo(f"   Run 'lovelace --upgrade' to update\n", err=True)
...
def init():
    ...
    try:
        ...
        for project in projects:
            ...
            # Format display name
            display_name = f"{project['name']:<25} [{project_type:<5}]"
            if description:
